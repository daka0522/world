...
class Cell {
    ...
    randomWalk() {
        // Choose -1, 0, or 1 for each axis
        const randomChoice = Array.from({length: 3}, () => Math.round(Math.random() * 2 - 1))
        const randomVec = new THREE.Vector3(...randomChoice).multiplyScalar(0.1) // for more smoother
        // console.log("R.V.: ", randomVec);
        
        this.velocity.add(randomVec) // Affect velocity, not position directly
    }
}
