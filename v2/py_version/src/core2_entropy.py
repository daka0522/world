...
class World: 
    ...
    def __init__(self, size: int=10) -> None:
        ...
        self.temperature: np.ndarray = np.zeros((self.HEIGHT, self.WIDTH), dtype=float)  # Temperature distribution
        self.energy_field: np.ndarray = np.zeros((self.HEIGHT, self.WIDTH), dtype=float)  # Energy distribution
        self.entropy: float = 0.0  # Total system entropy
        self.heat_diffusion_rate: float = 0.1  # Rate at which heat spreads
        self.energy_diffusion_rate: float = 0.1  # Rate at which energy spreads
        self.base_temperature: float = 20.0  # Base temperature in Celsius
        self.entropy_increase_rate: float = 0.01  # Rate at which entropy naturally increases
    ...
    def update_entropy(self) -> None:
        """Update the entropy of the system based on temperature and energy distribution."""
        # Calculate entropy based on temperature and energy distribution
        temp_entropy = np.sum(self.temperature * np.log(self.temperature + 1))
        energy_entropy = np.sum(self.energy_field * np.log(self.energy_field + 1))
        
        # Add natural entropy increase
        self.entropy += self.entropy_increase_rate
        
        # Update total entropy
        self.entropy = float(temp_entropy + energy_entropy) + self.entropy

    def diffuse_heat(self) -> None:
        """Diffuse heat across the world using a simple diffusion equation."""
        ...
        for i in range(1, self.HEIGHT-1):
            for j in range(1, self.WIDTH-1):
                ...
                new_temp[i,j] += self.heat_diffusion_rate * (
                    self.temperature[i-1,j] + self.temperature[i+1,j] +
                    self.temperature[i,j-1] + self.temperature[i,j+1] -
                    4 * self.temperature[i,j]
                )
        
        self.temperature = new_temp

    def diffuse_energy(self) -> None:
        """Diffuse energy across the world."""
        # Create a copy of the energy field
        new_energy = self.energy_field.copy()
        
        # Apply energy diffusion
        for i in range(1, self.HEIGHT-1):
            for j in range(1, self.WIDTH-1):
                # Simple diffusion equation
                new_energy[i,j] += self.energy_diffusion_rate * (
                    self.energy_field[i-1,j] + self.energy_field[i+1,j] +
                    self.energy_field[i,j-1] + self.energy_field[i,j+1] -
                    4 * self.energy_field[i,j]
                )
        
        self.energy_field = new_energy

    def update_physics(self) -> None:
        """Update all physics-related properties of the world."""
        self.diffuse_heat()
        self.diffuse_energy()
        self.update_entropy()
...
class Matter:
    ...
    def __init__(self, world: World) -> None:
        ...
        # Entropy-related properties
        self.heat_generation: float = 0.0  # Heat generated by this matter
        self.energy_output: float = 0.0  # Energy output by this matter
        self.decay_rate: float = 0.01  # Rate at which matter naturally decays
    ...
    def born(self, available_space: _location) -> None:
        if available_space is None:
            ...
            # Initialize temperature and energy at birth location
            if self.current_location is not None:
                row, col = self.current_location
                self.world.temperature[row, col] = self.heat_generation
                self.world.energy_field[row, col] = self.energy_output
    ...
    def die(self) -> None: 
        if self.is_alive and self.current_location is not None: 
            ...
            # Release remaining energy and heat upon death
            self.world.energy_field[row, col] += self.energy_output
            self.world.temperature[row, col] += self.heat_generation
    ...
    def update_entropy(self) -> None:
        """Update the matter's contribution to entropy."""
        if self.is_alive and self.current_location is not None:
            row, col = self.current_location
            
            # Apply natural decay
            self.heat_generation *= (1 - self.decay_rate)
            self.energy_output *= (1 - self.decay_rate)
            
            # Update world's temperature and energy fields
            self.world.temperature[row, col] += self.heat_generation
            self.world.energy_field[row, col] += self.energy_output
...
class Food(Matter):
    ...
    def __init__(self, world: World):
        ...
        self.energy: int = 20
        # Food-specific entropy properties
        self.heat_generation = 0.5  # Food generates less heat
        self.energy_output = 15.0  # Food has significant energy output
        self.decay_rate = 0.02  # Food decays faster than cells
...
class Cell(Matter): 
    ...
    def __init__(self, world: World) -> None:
        ...
        # Cell-specific entropy properties
        self.heat_generation = 2.0  # Cells generate more heat
        self.energy_output = 5.0  # Cells have moderate energy output
        self.decay_rate = 0.005  # Cells decay slower than food
    ...
    def move(self, new_location) -> None:
        if self.current_location is not None:
            old_row, old_col = self.current_location
            self.world.spaces[old_row, old_col] = 0
            
            # Leave some energy and heat behind when moving
            self.world.energy_field[old_row, old_col] += self.energy_output * 0.1
            self.world.temperature[old_row, old_col] += self.heat_generation * 0.1

        row, col = new_location
        self.world.spaces[row, col] = self
        self.current_location = new_location

        # Add energy and heat to new location
        self.world.energy_field[row, col] += self.energy_output
        self.world.temperature[row, col] += self.heat_generation
    ...
    def eat(self, food: Food) -> None:
        # Transfer energy and heat from food to cell
        self.energy += food.energy
        self.heat_generation += food.heat_generation * 0.5  # Some heat is lost in the process
        self.energy_output += food.energy_output * 0.7  # Some energy is lost in the process
        
        # Food releases its remaining energy and heat upon being eaten
        if food.current_location is not None:
            row, col = food.current_location
            self.world.energy_field[row, col] += food.energy_output * 0.3
            self.world.temperature[row, col] += food.heat_generation * 0.5
    ...
    def aging(self) -> None:
        ...
        if self.is_alive and elapsed_time % 5:
            ...
            # Increase entropy-related properties with age
            self.heat_generation *= (1 + 0.01 * self.age)  # Heat generation increases with age
            self.decay_rate *= (1 + 0.005 * self.age)  # Decay rate increases with age
